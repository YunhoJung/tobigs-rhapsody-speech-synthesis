...
def _process_utterance(out_dir, wav_path, text, hparams):
    ...
    # We rescale because it is assumed in Wavenet training that wavs are in [-1, 1] when computing the mixture loss.
    # This is mainly coming from PixelCNN implementation. https://github.com/Rayhane-mamah/Tacotron-2/issues/69
    ...
    # The quantization noise is from the analog to digital conversion. The mu-law
    # compression actually reduces the noise and increases the dynamic range. If you search a little bit in the code
    # you will find that the input is always mu-law encoded here. scalar_input only determines if the model uses a
    # one-hot encoding for every data point of the input waveform, or just uses floating point values for each sample.
    ...
    if mel_frames > hparams.max_mel_frames and hparams.clip_mels_length:
        # hparams.max_mel_frames = 1000,
        # hparams.clip_mels_length = True
        return None
    ...
    if npz_flag:
        ...
        np.savez(os.path.join(out_dir, npz_filename), **data,
                 allow_pickle=False)
    else:
