...
class DataFeederWavenet(threading.Thread):
    def __init__(self, coord, data_dirs, batch_size, receptive_field, gc_enable=False, queue_size=8):
        ...
        self.max_frames = self.sample_size // self.hop_size
        ...
        self.data_dir_to_id = {data_dir: idx for idx, data_dir in enumerate(self.data_dirs)}
        self.path_dict = get_path_dict(self.data_dirs, np.max([self.sample_size, receptive_field]))
    ...
    def _get_next_example(self, data_dir):
        """Loads a single example (input_wav, local_condition,speaker_id ) from disk"""
